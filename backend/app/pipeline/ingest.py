...
def load_workers(path: str = None) -> pd.DataFrame:
    ...
    if missing:
        logger.warning(f"workers.csv missing columns: {missing}")
...
def load_wage_rates(path: str = None) -> pd.DataFrame:
    ...
    missing = expected_cols - set(df.columns)
    if missing:
        logger.warning(f"wage_rates.csv missing columns: {missing}")

    # Convert dates
    df['effective_from'] = pd.to_datetime(df['effective_from']).dt.date
    df['effective_to'] = pd.to_datetime(df['effective_to'], errors='coerce')

    # Fill NaT effective_to with 9999-12-31
    df['effective_to_filled'] = df['effective_to'].fillna(pd.Timestamp('9999-12-31')).apply(
        lambda x: x.date() if isinstance(x, pd.Timestamp) else x
    )
    df['effective_to'] = df['effective_to'].apply(
        lambda x: x.date() if isinstance(x, pd.Timestamp) and not pd.isna(x) else None
    )
...
def load_supervisor_logs(path: str = None) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    expected_cols = {'log_id', 'worker_name', 'worker_phone', 'supervisor_id',
                     'work_date', 'hours', 'vendor_app', 'entered_at'}
    missing = expected_cols - set(df.columns)
    if missing:
        logger.warning(f"supervisor_logs.csv missing columns: {missing}")
    return df
...
def load_bank_transfers(path: str = None) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    expected_cols = {'utr', 'worker_phone', 'worker_name', 'amount_paise',
                     'transfer_timestamp', 'account_last4'}
    missing = expected_cols - set(df.columns)
    if missing:
        logger.warning(f"bank_transfers.csv missing columns: {missing}")
    return df
    ...
    for _, row in logs_df.iterrows():
        ...
        await session.execute(
            text("""
                INSERT INTO shift_logs
                    (log_id, worker_id, raw_worker_name, raw_worker_phone,
                     supervisor_id, work_date, hours, vendor_app,
            ...
            """),
            {
                ...
                'entered_at': entered_at_ist.isoformat(),
...
async def process_bank_transfers(
    ...
    for _, row in transfers_df.iterrows():
        ...
        billing_period = transfer_dt.strftime('%Y-%m')
